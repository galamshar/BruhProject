...
def event_new(request):
    ...
    if request.method == 'POST':
        ...
        if event_form.is_valid():
            ...
            return HttpResponseRedirect("/bruhproject/event")
        else:
            messages.error(request, 'Event was not added. Correct form errors.')
...
def event_close(request, event_id):
    ...
    if event.end_time > timezone.now():
        messages.warning(request, 'You cannot lock an event that is still in progress!')
        return HttpResponseRedirect('..')
    ...
    if request.method == 'POST':
        ...
        if event_close_form.is_valid():
            ...
            for bet in bets:
                ...
                bet.open = False
                bet.save()
            messages.success(request, 'The event was closed in related bets - successfully settled')
            return HttpResponseRedirect("/bruhproject/event/" + event_id + "/")
        else:
            messages.error(request, 'Failed to close event.')
